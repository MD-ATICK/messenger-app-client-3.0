...
function Me({ setuserListActivetap }) {
  ...
  const avatarHanlderDirect = async (e) => {
    ...
    try {
      ...
      if (ibbStatus === 200) {
        const { data, status } = await axios.post(`https://faltu-serverside.vercel.app/api/messenger/avatarUpdate`, { avatar: url, userid: user._id }, { headers: { Authorization: `Bearer ${token}` } })
        if (status === 201) {
          setuser(data.user)
  ...
  const AvatarHanlder = async (userid) => {
    ...
    try {
      const { data, status } = await axios.post(`https://faltu-serverside.vercel.app/api/messenger/avatarUpdate`, { avatar: avatars.find((avatar) => avatar._id === selectedAvatar).avatar, userid: userid }, { headers: { Authorization: `Bearer ${token}` } })
      if (status === 201) {
  ...
  const usenameHanlder = async () => {
    ...
    try {
      const { data, status } = await axios.post(`https://faltu-serverside.vercel.app/api/messenger/usrenameUpdate`, { username, userid: user && user._id }, { headers: { Authorization: `Bearer ${token}` } })
      if (status === 201) {
  ...
  const AccessDeleteHanlder = async (ac) => {
    ...
    if (user) {
      try {
        ...
        const { data, status } = await axios.put(`https://faltu-serverside.vercel.app/api/messenger/accessRemove`, { ac: ac }, { headers: { Authorization: `Bearer ${token}` } })
        if (status === 201) {
